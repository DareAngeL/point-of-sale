...
export async function PrintRefundByDate(
  jspdf: any,
  reportType: string,
  formValue: any,
  reportData: any,
  ...
  const sortedDataByDate: any = _.groupBy(reportData, "trndte");
  const sortedDataByPayment: any = _.groupBy(reportData, "itmcde");

  for (const recordList of Object.values(sortedDataByDate) as any[]) {
    for (const [index, record] of recordList.entries()) {
      ...
      //   group indicator
      if (!itmcdeList.includes(record["trndte"])) {
        itmcdeList.push(record["trndte"]);
        initPrint.TOP += 2;
        jspdf.setFont("NotoSansCJKtc-Regular", "bold");
        jspdf.setFontSize(11);
        jspdf.text(
          formatDateToYYYYMMDD(record["trndte"]),
          arrayColumn[0],
          initPrint.TOP,
          "left"
        );
  ...
  for (const recordList of Object.entries(sortedDataByPayment) as any[]) {
