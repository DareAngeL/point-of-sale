...
export function VoidReceiptV2({ isReprint }: VoidReceiptProps) {
  const {header, dineType, footer, syspar} = useAppSelector(
    (state) => state.masterfile
  );
  const {orderDiscountByCode, specialRequest} = useAppSelector(
    (state) => state.order
  );
      ...
      return (
        <>
          <div className="flex justify-between">
            <p className=" ">{payment?.cardclass}</p>
            <p className=" ">
              {formatNumberWithCommasAndDecimals(payment.extprc as number, 2)}
            </p>
          </div>
          <div className="flex justify-between">
            <p className=" ">CARD NUMBER:</p>
            <p className=" ">
              ***{payment?.cardno!.substring(payment?.cardno!.length - Math.min(4, payment?.cardno!.length))}
            </p>
          </div>
          <div className="flex justify-between">
